...
const ResultsPage: React.FC<Props> = ({ onNavigate, result }) => {
  ...
  const data = {
    ...
    pathogen: result?.pathogen ?? "Pending Analysis",
    confidence: result?.confidence ?? 0,
    description: result?.description ?? "No specific description available.",
    treatments: result?.treatments ?? [],
    // Handle null or empty string image
    image: result?.image && result.image !== "" ? result.image : null,
  ...
  return (
    <div className="bg-background-light dark:bg-[#102216] min-h-screen flex flex-col relative overflow-x-hidden font-display text-white print:bg-white print:text-black">
      {/* Backgrounds */}
      <div
        className="fixed inset-0 z-0 opacity-20 pointer-events-none bg-[size:40px_40px] print:hidden"
        style={{
          backgroundImage:
            "linear-gradient(to right, #316843 1px, transparent 1px), linear-gradient(to bottom, #316843 1px, transparent 1px)",
        }}
      ></div>
          ...
          >
            ...
            <div className="text-left">
              <h1 className="text-white text-lg font-bold tracking-wider uppercase">
                Plant Disease{" "}
                <span className="text-primary font-normal">Detection</span>
              </h1>
              <p className="text-[10px] text-gray-400 tracking-[0.2em] uppercase leading-none">
                Analysis Report
              </p>
            </div>
          </button>
      ...
      <main className="relative z-10 flex-1 max-w-[1400px] mx-auto w-full p-6 lg:p-8 flex flex-col gap-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-white/5 pb-6 print:border-black">
          <div className="space-y-2">
            ...
            <h2 className="text-4xl font-bold tracking-tight text-white print:text-black">
              Diagnosis Results
            </h2>
            <p className="text-gray-400 max-w-xl print:text-gray-600">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-12 glass-panel rounded-2xl p-6 relative overflow-hidden group border border-white/5 shadow-lg print:border-black print:shadow-none">
            <div className="flex flex-col md:flex-row gap-8 items-center h-full">
              ...
              <div className="relative shrink-0 w-32 h-32 md:w-48 md:h-48 rounded-xl overflow-hidden border-2 border-danger/30 shadow-[0_0_20px_rgba(239,68,68,0.2)] print:border-black print:shadow-none bg-black/40 flex items-center justify-center">
                {data.image ? (
                  <div
                ...
                <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-danger text-white text-[10px] font-bold uppercase rounded print:border print:border-black print:text-black print:bg-white">
                  Pathogen Detected
                </div>
              </div>
              ...
              <div className="flex-1 space-y-4 text-center md:text-left">
                <div>
                  <div className="text-gray-400 text-xs font-mono uppercase tracking-wider mb-1 print:text-gray-600">
                    Detected Disease
                  </div>
                  <h3 className="text-3xl md:text-4xl font-bold text-danger neon-text-glow print:text-black print:shadow-none">
                    {data.name}
                  </h3>
                  <p className="text-gray-400 text-sm mt-2 italic print:text-gray-800">
                    {data.pathogen}
                  </p>
                </div>
                <p className="text-sm text-gray-400 line-clamp-3 print:text-black">
                  {data.description}
                </p>
              </div>
              ...
              <div className="shrink-0 w-32">
                <div className="relative">
                  <svg className="circular-chart" viewBox="0 0 36 36">
                    <path
                      className="circle-bg print:stroke-black"
                      d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                    ></path>
                    <path
                      className="circle print:stroke-black"
                      d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                      strokeDasharray={`${data.confidence}, 100`}
                    ></path>
                    <text
                      className="percentage print:fill-black"
                      x="18"
                      y="20.35"
                    >
                      {data.confidence}%
                  ...
                  <div className="text-center mt-2 text-[10px] text-gray-400 uppercase tracking-wider print:text-black">
                    Confidence
                  </div>
                </div>
              </div>
          ...
          {data.treatments.length > 0 && (
            <div className="lg:col-span-12 glass-panel rounded-2xl p-6 md:p-8 border border-white/5 print:border-black print:text-black">
              <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-3 print:text-black">
                <span className="material-symbols-outlined text-caution print:text-black">
                  medical_services
                </span>
                Treatment Protocol
              </h3>
...
const TreatmentCard: React.FC<{
  ...
  // FIX: Fallback to 'primary' if the API returns a color we don't know
  // This prevents the "Black Screen" crash
  const selectedColorKey = colorClasses[color] ? color : "primary";
  const styles = colorClasses[selectedColorKey];

  // Helper to split styles safely
  const getStyle = (idx: number, endIdx?: number) =>
    styles.split(" ").slice(idx, endIdx).join(" ");

  const badgeClass = `text-[10px] border px-2 py-0.5 rounded uppercase font-bold ${getStyle(0, 2)} print:text-black print:border-black`;
  const iconClass = `p-2 rounded-lg ${getStyle(0, 1)} ${getStyle(2, 3)} print:hidden`;
  const listClass = `text-sm text-gray-400 space-y-2 list-disc list-inside ${getStyle(3)} print:text-black`;
  const borderHover = `hover:border-${selectedColorKey}/30`;

  return (
    <div
      className={`bg-surface-dark/50 p-5 rounded-xl border border-white/5 ${borderHover} transition-all group print:bg-white print:border-black`}
    >
...
const ExternalLink: React.FC<{
  ...
  desc: string;
  url: string;
}> = ({ icon, color, title, desc, url }) => {
  const colorMap: Record<string, string> = {
    blue: "text-blue-400 bg-blue-900/30 border-blue-500/30 group-hover:text-blue-400",
    purple:
      "text-purple-400 bg-purple-900/30 border-purple-500/30 group-hover:text-purple-400",
  };

  // Safe fallback for color map
  const safeColor = colorMap[color] ? color : "blue";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <div
          className={`size-12 rounded-full flex items-center justify-center border ${getStyle(0, 3)}`}
        >
          <span className="material-symbols-outlined">{icon}</span>
        </div>
        <div>
          <h4
            className={`text-white font-bold ${getStyle(3)} transition-colors`}
          >
            {title}
          </h4>
